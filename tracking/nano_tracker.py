...
class _NanoInternalTrack:
    def __init__(
        ...
        assert track_id is not None, "track_id must be provided (use NanoTracker._next_track_id for new tracks)"
        ...
        self.was_reidentified = track_id is not None
        ...
        for det in detections:
            ...
            if best_iou >= self.iou_threshold and best_track is not None:
                ...
                if det.get("conf") is not None:
                    best_track.confidence = float(det["conf"])
                ...
                reid_track = self._try_reid_match(frame, det_bbox, cls_val)
